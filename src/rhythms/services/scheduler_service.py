...
class SchedulerService:
    ...
    def prepare_and_notify(self, user_id: str = None, channel_id: str = None):
        """Execute standup preparation and notification."""
        try:
            # Use instance variables if not provided
            if user_id is None:
                user_id = next(iter(self.jobs.keys()))  # Get the first user_id
            if channel_id is None:
                channel_id = os.getenv("SLACK_CHANNEL_ID")  # Fallback to env var
            ...
            if result:
                ...
                try:
                    ...
                    if update_result:
                        ...
                        # Send the final standup to Slack
                        if "FINAL STANDUP:" in str(update_result):
                            self.slack_bot._send_to_slack(
                                channel_id,
                                str(update_result),
                                thread_ts
                            )
                            logger.info("Final standup sent to Slack")
    ...
    def schedule_standup(self, user_id: str, channel_id: str, notification_time: str = "10:00"):
        ...
        job = schedule.every().day.at(notification_time).do(
            self.prepare_and_notify, 
            user_id=user_id, 
            channel_id=channel_id
        )
